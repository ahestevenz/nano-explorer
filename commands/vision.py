...
def register(parser: argparse.ArgumentParser, settings: NanoSettings):
    ...
    p_det.add_argument(
        "--stream-port", type=int, default=settings.stream_port, dest="stream_port", metavar="PORT"
    )
    ...
    # track
    p_track = sub.add_parser("track", help="Track objects, colours or blobs and steer the robot")
    p_track.add_argument(
        "--mode",
        choices=["color", "blob", "object"],
        default="color",
    ...
    # gesture
    p_gest = sub.add_parser("gesture", help="Control the robot with hand/body gestures (trt_pose)")
    p_gest.add_argument(
        "--config",
        default=str(settings.pose_config_path),
        dest="config_path",
